...
import { useMonthlyComplaintsContext } from "./context/MonthlyComplaintContext";
import { MonthlyComplaint } from "./MonthlyComplaintData";
import AddMonthlyComplaint from "./context/AddMonthlyComplaint";
import EditMonthlyComplaint from "./context/EditMonthlyComplaint";




const MonthlyComplaintTable = () => {
    const {
        page,
        pageSize,
        searchQuery,
    ...
    const handleEditSubmitForm = (payload: MonthlyComplaint) => {
        UpdateMonthlyComplaint(payload);
        setisEdit(false);
    };
    const handleAddSubmitForm = (payload: MonthlyComplaint) => {
        crateMonthlyComplaint(payload);
        setisAdd(false);
    };
                        ...
                        <Table sx={{ minWidth: 650 }} aria-label="simple table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="left">CarriedForwardFromPreviousMonth</TableCell>
                            ...
                            <TableBody>
                                <>
                                    {tableData && tableData.length > 0 ? (
                                        tableData.map((row: MonthlyComplaint) => (
                                            <TableRow
                                                key={row._id}
                                                sx={{ "&:last-child td, &:last-child th": { border: 0 } }}
                                            >
                                                <TableCell align="left">{row.ReceivedFrom}</TableCell>
                                                <TableCell align="left">{row.CarriedForwardFromPreviousMonth}</TableCell>
...
export default MonthlyComplaintTable;
