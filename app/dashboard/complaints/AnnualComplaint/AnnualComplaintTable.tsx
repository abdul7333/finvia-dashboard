...
import { useAnnualComplaintContext } from "./context/AnnualComplaintContext";
import { AnnualComplaint } from "./AnnualComplaintData";
import AddAnnualComplaint from "./context/AddAnnualComplaint";
import EditAnnualComplaint from "./context/EditAnnualComplaint";
...
const AnnualComplaintTable = () => {
    ...
    const handleEditSubmitForm = (payload: AnnualComplaint) => {
        UpdateAnnualComplaint(payload);
        setisEdit(false);
    };
    const handleAddSubmitForm = (payload: AnnualComplaint) => {
        crateAnnualComplaint(payload);
        setisAdd(false);
    };
    ...
    return (
        <div>
                        ...
                        <Table sx={{ minWidth: 650 }} aria-label="simple table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="left">CarriedFromPrevMonth</TableCell>
                            ...
                            <TableBody>
                                <>
                                    {tableData && tableData.length > 0 ? (
                                        tableData.map((row: AnnualComplaint) => (
                                            <TableRow
                                                key={row._id}
                                                sx={{ "&:last-child td, &:last-child th": { border: 0 } }}
                                            >
                                                <TableCell align="left">{row.ReceivedFrom}</TableCell>
                                                <TableCell align="left">{row.CarriedFromPrevMonth}</TableCell>
            ...
            AnnualComplaints={AnnualComplaints}
            />
        </div>
    );
};
