...
const EditClientComplaints  = ({
  open,
  setOpen,
  handleSubmitForm,
  clientComplaints,
}: Props) => {
  const [formValues, setFormValues] = useState<ClientComplaint | null>(null);

  useEffect(() => {
    if (open && clientComplaints) {
      setFormValues(clientComplaints);
    } else {
      setFormValues(null);
    }
  }, [open, clientComplaints]);
...
export default EditClientComplaints;
