...
import EditUser from "./context/EditUser";
import Image from "next/image";
import { Height } from "@mui/icons-material";
import EditPassword from "./context/EditPassword";
...
const UserTable = () => {
    ...
    const handleEditSubmitForm = (payload: User) => {
        updateUser(payload);

        setisEdit(false);
    };

    const handleEditPasswordForm = (payload: User) => {
        updatePassword(payload);

        setisEditPassword(false);
    };

    const handUserSubmitForm = (payload: User) => {
        createUser(payload);
        setisAdd(false);
    };
    ...
    const getrole = (_id: string) => {
        const role = roles?.find((el) => el.value.toString() === _id.toString());
        ...
        <div>
            <div className={styles.TableFlexTop} >
                ...
                <div className={styles.TableButtonContainer}>
                    ...
                    <div>
                        {myPermissions?.permission.users.create ? (
                            <Button
                                variant="contained"
                                    ...
                                    {tableData && tableData.length > 0 ? (
                                        tableData.map((row: User) => (
                                            ...
                                            >
                                                ...
                                                <TableCell align="right">
                                                    {myPermissions?.permission.users.edit ? (
                                                        <Button
                                                            onClick={() => {
                                                                setUser(row);
                                                        ...
                                                        >
                                                            <Image src={'/change-password-icon.svg'} alt="change" width={25} height={25} />
                                                        </Button>
                                                    ) : (
                                                        <Button
                                                            disabled
            ...
            <AddUser
                ...
                setOpen={setisAdd}
                rolesOptions={roles}
                handleSubmitForm={handUserSubmitForm}
            />
            <EditUser
                open={isEdit}
                setOpen={setisEdit}
            ...
            <EditPassword
                open={isEditPassword}
                setOpen={setisEditPassword}
                handleSubmitForm={handleEditPasswordForm}
                user={user}
            />
        </div>
    );
