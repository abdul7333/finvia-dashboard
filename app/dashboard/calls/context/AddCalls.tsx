...
const AddCalls  = ({ open, setOpen, handleSubmitForm }:Props ) => {
  const initialFormValues: Calls = {
    stock: "",
    action: "",
    type: "",
    quantity: 0,
  ...
  const [formValues, setFormValues] = useState<Calls>(initialFormValues);
  ...
  return (
    <Drawer  open={open} onClose={() => setOpen(false)} anchor={"right"} sx={{overflow:"hidden"}}>
              ...
              <Col>
                <FormGroup>
                  ...
                  <Input
                    ...
                    type="number"
                    disabled
                    value={((formValues.booked-formValues.entry)*formValues.quantity).toFixed(0)}
                    // onChange={handleInputChange}
                  />
                </FormGroup>
              </Col>
